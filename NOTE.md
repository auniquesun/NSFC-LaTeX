...
8. 文中一会儿用“点云分析”，一会儿用“点云感知”，挺晕乎的，应该统一一下                                                                       - 这个等下文一起改吧
        ...
        - 【问题】但目前**只有2大块研究内容**
            1. 第一块是点云特征编码
                ...
                - 这块还得思考一下，走哪条技术路线？
                    1. 对比预训练
                        - 这么做会不会被认为创新性不够？
                    2. 借助 2D open-set model 给点云分配语义标签
                        - 这就不涉及训练了？**和点云特征编码的主题不太搭**

            2. 第二块是在线**动态分层缓存的构建和更新**
                - 这里的目标是理解在线测试数据的特性
            ...
            3. 最后一块研究内容是什么呢？
                ...
                - 调研一下新的论文，按照每年发表顺序
                    - ICML，2024.2    - 2024
                    - ICCV，2025.3月，奇数年    - 2025
                    - ECCV，2024.3月，偶数年    - 2024
                    - MM, 2024.4月      - 2024
                    - NeurIPS，2024.5月 - 2024
                    - AAAI，2024.8月    - 2025
                    - ICLR，2024.9月    - 2025
                    - CVPR，2024.11月   - 2025
                - 【已解决】基于动态分层缓存模型的测试时自适应
                    - 内容部分确定了，要找对应的技术实现
                    - **检索增强的测试时自适应**
...
4. 重写国内外研究现状
    - 研究目标、研究内容、关键科学问题可以先写出来
    - 把研究方案写完再写这块也不迟，先得针对性读一些文献
...
5. 研究内容确定和展开
    1. 基于对比预训练的点云编码方法
        - 这个思路倒是比较直观，也比较适合识别任务，但是否具有创新性？
            - 2-3年前比较流行，现在属于比较老的思路了，新思路有哪些？
                - Open YOLO 3D, ICLR 2025
                - Open3DIS, CVPR 2024
                - OpenMask3D, NeurIPS 2023
                    - pipeline和论文描述非常清楚，直观易懂，以此为基础去扩展，扩展时参考 Open YOLO 3D
        
        - 到底选不选这个思路
            - 我觉得要选个新的思路

6. 关键科学问题
    - 在研究背景部分已经总结过一次了，先拿过来再丰富
    - 已完成

7. 重新审视研究方案
    0. 这部分内容要连贯、充实
    1. 先把第一部分的方案确定下来，填上内容 
        - 备选方案
            1. 大规模点云-文本-图像数据对比预训练
                - 【放弃】单纯做识别可以，检测和分割还不够，这是局限性

            2. 将点云投影到2D平面，再利用开放词表的图像模型处理
                - 再详细读一下相关论文
                - 【已确定】走这个路线，以 OpenMask3D为例，要确定分哪几步
                    1. 输入 point cloud + posed RGB-D frames
                    2. 类别无感的3D物体掩码生成
                    3. 3D物体掩码特征计算
                    4. 开放词表的3D物体实例分割

        - 我在思考一个问题，就是技术方案要完全copy别人的工作吗？一是这样写没有创新性，二是显得已经把东西做出来了
            - 目前的情况，先写一版出来是关键，后面可以在这个基础上改
            - 【**今天继续完善这块，要串成一个故事，不能分崩离析**】
                1. 接着改动态分层缓存 + 测试时自适应策略
                2. 点云特征编码 + 动态分层缓存 **两部分符号不统一**
                    - 着力解决此事
                    - 【已完成】

        - 史院长讲了，*青年基金要大胆创新，大胆提新想法新方案*
            - 创新得有想法有技术，凭空产生不了创新
            - 想法还得读文献+实践啊，**跳出 point cloud test-time adaptation 小领域，关注一些领域内重大成就和进展**

        - 目前自己回看一遍，写得就像一坨shit
        - 下一步该写啥了？
            - 写的时候要结合国自然新模板，**不能完全依赖2025年及以前的模板了**
            - 可行性分析还是需要的，这部分当最后的可行性分析去写
                1. 这部分和研究基础还不一样，研究基础是写前期的论文发表积累的东西
                2. 可行性分析是对立项依据、技术方案能否成功实施的分析
                3. 在新模板页数限制下，这个分析写一段即可，按照夏老师的写法，肯定打不住
                4. 现在要明确存在什么问题，知道存在的问题才能去改
                    - 对于具体技术路线，支撑技术和材料得加一点，干巴巴说之前研究过相关的有点乏力

8. 创新点重新总结了一下

9. 年度研究计划要写一稿
    - 这次要做个甘特图出来，这是必要的
        - 在哪里做？excel 就可以吧，但外围线条需要去除一下
        - 关键的是确定内容

10. 研究基础，该重新写一版了
    - 这块应该怎么写？
        1. 首先自己应该思考，我做这个项目，有哪些相关积累？
            - 发表的论文都是科研积累，写的时候要扣住项目主题
            - 在项目中负责的部分，要优化一下，目前写得太浅
            - 研究获奖情况
                1. 这里写空间智能是否合适？
                    - 只在立项依据提过空间智能，这里突然提有点别扭
                2. 有几个写了获奖但证书没放，可以吗？
                    - 先把文字部分写好了，放点代表性的就可以，不必全放
        2. 其他人的申请书此部分？有哪些好的做法，我有什么遗漏部分？
            - 所属团队介绍
                - 史院长、时空智能研究中心
                - 先介绍这部分吗？**可以先写后面的工作条件**
            - 预研工作加个参考文献
...
1. 重新规划研究内容
    ...
    - 解决思路是什么？
        - 读文献、大胆做新的东西
        - 之前的工作在对“开放集点云感知”问题上，普遍认为需要在基类上训练，再拿到新类别测试
            1. 但是仔细一想，即使是“开放集”，**总类别数也没有那么多，仅在有限基类上训练反而会拉低模型效果**
                - 根据这个想法做工作
            2. 史振威院长之前也讲过，遥感中低轨影像，做开放集识别，也就是有限的类别，**不超过100类，设定几万个类别意义不大**
            3. 这让我联想到了 Tesla FSD，真做的挺牛的，在标准路上基本不用人管了，国内厂商大概也做到了吧，之前看过小米汽车的视频，差距没那么大
        - 列了这么多事项，到底先做哪项？一项项来
            - 先改进一下创新性的问题
            - 提出研究项目的时候，可以从积累的【未来研究方向】找前沿问题
            - 换研究内容，换成什么呢？还是在现有基础上做吧

2. 该开始画图了
    - 研究内容定了再画
    - 开始画了，不能拖了

3. 找个NSFC 2026的 latex 模板，形成完整一稿
    1. [仓库](https://github.com/fylimas/nsfc)中说形式审查没问题
        1. 章节标题有点问题？
        2. 到时候贴进去文本试一下
    2. [Online latex project](https://www.onlinelatex.com/project/697085f5b507200f4a4e7c36)
        1. 到时候贴进去文本试一下
    3. 果断放弃南开大学CMM这个模板吧，所有内容都堆在一起，而且格式和官方有明显出入，**不符合要求**

4. 国内外研究现状这块还没写，要写一个正式的版本，让申请书文字部分形成一个版本
    - 目前这块还是最初的版本，单纯放在这了，缺乏体系逻辑
        - 把原来写的不成体系的 **国内外研究现状** 挪到单独 `related_work.txt` 了
    - 分成哪几部分写？
        1. 开放环境下3D点云感知
        2. 测试时自适应
        3. 缓存模型

        - 上面几部分是原来设定的，显式分成这几部分，**这种模式像是写论文**；但看了几个国自然本子，大家都是串成一个故事
            - 分主题逻辑会比较清晰，但每部分内容应当尽量充实，现在太少了，刘瑞和夏日婷都写了5-6页
            - 怎么充实起来？
                - 一方面根据研究主题调研文献总结
                    - 目前写的东西太老了，还是 CLIP、ALIGN
                - 二方面适当借助大模型
        
5. 国内外研究现状卡壳太久了，要开始画图了
    1. 先画什么图？
        0. 点云感知模型自适应饮用
        1. 研究内容整体框架，这个图非常重要，在纸上画个草图
        2. 技术路线图
        3. 三部分研究内容，各画一个图