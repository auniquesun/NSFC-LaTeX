...
\myEmph{挑战2:} 现有方法未能设计有效的测试时自适应策略，模型在推理部署阶段缺乏应对未知环境数据噪声、数据坍塌、分布偏移的能力，开放环境中性能急剧下降，难以满足实际应用对准确性、鲁棒性、高效性的要求。
...
%   目前只有2个研究点，那就把这2个写出来
%   写的思路是什么？参考下别人的
在此背景下，本项目应运而生，拟采用循序渐进的方式研究开放环境下3D点云感知方法的自适应问题。
首先探讨具有语义理解能力的点云编码问题，赋予3D点云感知模型识别新物体、新类别、新场景的基础能力；在此基础上，探讨已经训练好的点云感知模型在测试和部署阶段，如何根据在线测试数据特点作出自适应调整，实现更好的测试时准确性和鲁棒性。

\myEmph{关键科学问题1}：在模型训练阶段，如何有效编码点云数据，使得模型具有开放环境的语义理解能力？

\myEmph{关键科学问题2}：在模型测试阶段，如何有效应对未知场景的数据噪声、坍塌、分布偏移问题？
