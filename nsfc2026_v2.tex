...
% 怎么和上述概览的逻辑接起来
%   介绍典型任务分类
开放环境下3D点云处理典型的任务包括点云识别~\cite{zhang22pointclip,zhu23pointclip2,sun25pointcache,sun24point-prc,sun24ppt3d,xue23ulip,xue24ulip2,liu23openshape,zhou24uni3d}、
...
点云物体检测~\cite{zeng23clip2,zhu23pointclip2,lu23ov3det,yang24imov3d,cao23coda,cao25codav2}、
...
点云语义分割~\cite{jiang24ov3d,kolodiazhnyi2024oneformer3d,zhou2025pointsam,rusnak25haec,boudjoghra2025openyolo3d}等。
...
这几类任务比较起来有很大差异，从数据规模和复杂度层面，3D点云识别涉及到物体级的点云数据，规模和复杂度相对较小，而3D物体检测和语义分割要处理场景级的点云数据，规模和复杂度较大，因此不同任务的处理方法也有较大差异。
...
测试时自适应技术在图像处理领域已经得到体现和应用。早期的图像处理测试时自适应技术应用在图像单一模态~\cite{liang20shot,liang22source,wang21tent,zhang22memo,sun20ttt,liu21ttt++,gandelsman22ttt-mae,iwasawa21t3a,zhang22memo}，由于未引入灵活的语言模态，这类方法只能在封闭数据集上进行图像识别，不具备在真实开放世界中进行图像识别的能力。近年来，测试时自适应技术与主流视觉-语言模型结合，掀起了开放环境下多模态大模型测试时自适应方法研究的浪潮~\cite{shu22tpt,feng23difftpt,tsai24gda,karmanov24tda,zanella24meanshift,farina24zero,guan2025sca,cha2025snap}，这些工作大大突破了封闭数据集的设定，使得视觉-语言多模态模型更加贴近实际应用。
...
与在图像处理领域的探索相比，3D点云感知测试时自适应技术起步较晚，相关研究仍属于早期阶段。当前的技术流派可大致划分为三大类：第一类是基于小模型的方法~\cite{mirza23mate,wang24bftt3d,shim24cloudfixer}，这类方法首先局限于单一点云模态，缺乏开放集合的3D点云处理能力，其次由于模型本身能力有限，虽有测试时自适应技术加持，但展现的准确性和泛化能力不够；第二类是基于多模态大模型的方法~\cite{sun25pointcache,tamjidi2025uniadapter}，这类方法虽然能进行开放集合的点云处理，但是没有充分考虑测试数据的分布特性，导致模型无法很好适应测试环境，在现实世界测试和部署时鲁棒性和泛化能力仍不理想。
...
这些问题限制了缓存模型与测试时自适应方法的有效结合，使得基于缓存模型的3D点云自适应分析成为一大技术难点。为此，本项目提出了基于测试时分层缓存模型的开放环境下3D点云感知自适应解决方案，该方案设计的缓存模型（1）能够在线捕捉测试数据的分布特性，
...
\begin{figure}[t]
	\centering
    \includegraphics[width=\columnwidth]{figures/research_framework.pdf}
    \caption{本项目研究内容整体框架。}
    \label{fig:teaser}
\end{figure}
...
申请人自2017年开始从事基于深度学习的计算机视觉和三维视觉相关研究工作，具备扎实的理论基础和丰富的实践经验，在基于深度学习的点云理解方法研究中取得了系列创新成果，受邀担任TPAMI, IJCV, CVPR, ICCV, NeurIPS等领域内顶级期刊和会议的审稿人。申请人的博士学位论文《多视图多模态三维物体理解关键技术研究》获评中国人民大学2025年度优秀博士学位论文，申请人于2023年获得国家留学基金委公派留学奖学金，2024年赴澳大利亚莫纳什大学（Monash University）数据科学与人工智能系进行为期一年的博士联合培养，联培期间的研究方向仍是三维视觉和点云处理，导师为蔡剑飞教授（Prof. Jianfei Cai），联培期间与蔡剑飞教授团队建立了良好而深入的合作关系。下文将从申请人的前期预研工作、参与的相关项目、获得的相关奖励三方面介绍研究基础：
...
\subsection{申请人所属团队情况}
% 时空智能中心
%   实验场地
申请人是内蒙古大学时空智能研究中心团队成员，该中心由\myEmph{内蒙古大学计算机学院特聘院长、国家杰出青年基金获得者史振威教授}领衔。时空智能研究中心聚焦智慧生态、边防、能源、交通、应急等特色应用场景，深耕具有内蒙古地域特色的时空智能理论研究与技术攻关。本项目围绕开放环境下3D点云感知方法的自适应问题开展研究，紧扣时空智能研究中心主题，是团队在地理空间数据智能处理方面的一个重点方向。
...
% 学校介绍
申请人的工作单位是内蒙古大学计算机学院，内蒙古大学作为国家“211工程”重点建设院校和国家“双一流”建设高校，具有较强的综合实力和较好的研究平台，是中国教育科研计算网（CERNET）内蒙古地区主节点。
% 这部分的必要性有待定夺？讲博士点干嘛？说明学校还不错啦
内蒙古大学计算机学院现有计算机科学与技术一级学科博士点和博士后科研流动站，设有“蒙古文智能信息处理技术国家地方联合工程研究中心”和“生态大数据教育部工程研究中心”两个国家级和部委级科研平台。

%   实验设备
实验设备方面，申请人购买了有1个台式电脑、1个笔记本电脑、1台打印机。台式电脑配置为Intel Core i9-14900K CPU，192GB内存，2张NVIDIA RTX 5090显卡，12TB 硬盘存储空间；笔记本电脑配置为 Apple M4 Max 处理器，128GB内存，4TB固态硬盘。此外，时空智能研究中心有8台浪潮服务器，每台配置了4块 A100 GPU，申请人可以根据项目需求进行使用。这些设备为本项目数据存储和处理、模型设计、方案验证和迭代优化提供了稳定的硬件条件和算力支撑。

% 软件和数据
软件工具方面，申请人掌握了常用的开发工具，如Python, C++, PyTorch以及针对点云和图像处理的专用软件，比如PyTorch3D\footnote{\url{https://github.com/facebookresearch/pytorch3d}}, Torch-Points3D\footnote{\url{https://github.com/torch-points3d/torch-points3d}}, Open3D\footnote{\url{https://github.com/isl-org/Open3D}}，
具备了利用Linux、Docker、Git等操作系统和工具搭建实验环境的实际经验，
能够自主实现点云感知模型的设计、训练、调试和可视化，这些基础能力为本项目研究内容的深入推进提供了有力支撑。

% 数据集
数据资源方面，申请人在读博士期间搜集了领域内大量公开的点云数据集，熟悉数据集的下载地址、存储格式和使用方法。点云物体识别方面，公开的数据集包括 ModelNet\footnote{\url{https://modelnet.cs.princeton.edu/}}、ScanObjectNN\footnote{\url{https://hkust-vgd.github.io/scanobjectnn/}}、ShapeNet\footnote{\url{https://shapenet.org/}}、OminObject3D\footnote{\url{https://omniobject3d.github.io/}}、Objaverse\footnote{\url{https://objaverse.allenai.org/}} 等；点云物体检测方面，分为室内场景数据（ScanNet\footnote{\url{http://www.scan-net.org/}}、SUN RGBD\footnote{\url{https://rgbd.cs.princeton.edu/}}、S3DIS\footnote{\url{https://sdss.redivis.com/datasets/9q3m-9w5pa1a2h}}、ARKitScenes\footnote{\url{https://github.com/apple/ARKitScenes}}、MultiScan\footnote{\url{https://3dlg-hcvc.github.io/multiscan/\#/}}、3RScan\footnote{\url{https://waldjohannau.github.io/RIO/}}）和室外场景数据（KITTI\footnote{\url{https://www.cvlibs.net/datasets/kitti/}}、nuScenes\footnote{\url{https://www.nuscenes.org/}}、Waymo Open\footnote{\url{https://waymo.com/open/}}、ONCE\footnote{\url{https://once-for-auto-driving.github.io/}}、Argoverse 2\footnote{\url{https://www.argoverse.org/av2.html}}）；点云场景语义分割方面，有室内场景数据集（S3DIS、ScanNet、Matterport3D\footnote{\url{https://niessner.github.io/Matterport/}}、Structured3D\footnote{\url{https://structured3d-dataset.org/}}、Replica\footnote{\url{https://github.com/facebookresearch/Replica-Dataset}}）和室外场景数据集（SemanticKITTI\footnote{\url{https://semantic-kitti.org/}}、Semantic3D\footnote{\url{https://semantic3d.ethz.ch/}}、PandaSet\footnote{\url{https://pandaset.org/}}、nuScenes (LidarSeg)、Waymo Open Dataset）。此外，申请人所在的时空智能研究中心配备了2架搭载激光雷达扫描仪、可见光和红外光相机的DJI专业无人机，可以根据需要提出申请，对内蒙古特定地域进行点云和图像数据采集，用于真实场景点云感知模型的训练和验证。
这些已有的数据积累和配备的数据采集设备为本项目探索创新性想法、比较不同研究方案的优劣以及迭代优化奠定了坚实的数据基础。

% 其他实验设备/耗材/劳务/差旅/专家咨询/论文版面和专利费
本项目计划购买GPU服务器一台，用于满足点云感知模型的训练和自适应加速需求，用到的其他小型设备、实验耗材，以及研究人员劳务、参加学术会议、专家咨询、论文发表和申请专利涉及的费用，拟通过本项目申请的专项经费予以支持。
