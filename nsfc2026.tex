...
\begin{itemize}
    \item 研究内容三：开放环境下缓存模型驱动的3D点云自适应分析方法。建立起缓存模型后，缓存模型中维护的点云样本特征和标签信息可作为知识锚点，在此基础上本项目计划研究一种检索增强技术，根据当前输入样本从缓存模型中检索相关领域知识，利用检索到的知识更新和校正3D点云分析模型原本预测，实现检索-增强目标。
\end{itemize}
