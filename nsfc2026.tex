...
% 3D点云+ 3D感知任务的重要性 
在数智化浪潮席卷全球的今天，如何精准地表征与感知真实物理世界，是机器视觉与人工智能领域面临的重要挑战。3D点云（***有必要画个图讲一下？***）作为物理世界最直接、最精确的数字表征形态，相较于二维图像，蕴含着不可替代的几何结构与深度信息，能够有效克服光照变化、尺度歧义等局限。因此，基于3D点云的高精度感知任务
——涵盖三维物体识别、检测与场景语义分割——构成了机器理解复杂物理现实的核心能力，是连接数字虚拟空间与实体物理世界的关键纽带。
...
% 讲空间智能的重要性
“空间智能”这一前沿概念由世界知名人工智能专家、斯坦福大学教授李飞飞率先提出，被视为系语言智能之后，人工智能发展的下一个重要里程碑。李飞飞教授强调了AI系统不仅要处理符号和二维图像，更要在真实的三维物理世界中具备感知、理解、推理与交互的核心能力。当前，国内外术界与产业界已形成广泛共鸣：人工智能必须走出屏幕，走向物理实体，发展能够适应复杂物理环境的智能。
...
% 过渡到空间智能
% 2024年初，世界著名人工智能专家、斯坦福大学李飞飞教授率先提出了“空间智能 (Spatial Intelligence)”这一前沿概念，强调了AI系统在三维世界中进行感知、理解、推理与交互的核心能力，并将其视为人工智能发展的下一个重要里程碑。
% 同年，李飞飞教授主导成立了 World Labs，致力于空间智能相关的研究和产品开发。

% 如何过渡到本项目的挑战？
然而，将空间智能系统真正部署于开放、动态的现实物理环境（Open Environment）中，面临着巨大的挑战。现有模型大多基于封闭数据集训练，一旦遇到未知的场景、天气或传感器噪声，往往因数据分布差异而导致性能显著下降，难以满足实际工业与社会应用对准确性、鲁棒性与安全性的严苛要求。

正是在此背景下，本项目《开放环境下3D点云自适应分析方法研究》应运而生。
...
总体目标：本项目旨在研究开放环境下3D点云处理方法测试时自适应问题，首先提出点云特征层级式编码方法加强点云特征表达能力；其次构建一个动态分层缓存模型，该缓存模型相当于一个3D点云知识库，能够实时捕获测试数据流的关键分布特性，并且能够根据测试数据特点变化更新自身数据，以更好适应测试和部署阶段出现的新特点新变化；最后设计基于缓存模型的检索增强机制，根据测试数据输入从缓存模型检索相关知识，利用检索到的知识校正和补充模型原有预测，最终实现开放环境下3D点云处理方法在测试环境的准确性和鲁棒性显著提升。
...
针对当前方法不能很好应对开放环境下3D点云数据的高度不确定性（物体类别数未知且动态变化、
数据噪声大、分布差异大），点云特征表示不全面不准确，模型准确性和泛化性差等问题，
本项目计划层层递进研究三方面内容：（1）开放环境下3D点云测试数据流特征编码；
（2）开放环境下缓存模型的动态构建与更新；（3）开放环境下缓存模型驱动的3D点云自适应分析方法

\begin{itemize}
    \item 研究内容一：开放环境下3D点云测试数据流特征编码。针对当前方法对点云特征编码能力不足的问题，本项目计划设计全局结合局部的点云特征编码方法，既考虑点云数据整体结构信息，也考虑局部组成信息，从而加强对测试时点云数据流的特征刻画和表达。
\end{itemize}


\begin{itemize}
    \item 研究内容二：开放环境下缓存模型的动态构建方法与更新机制。针对在线测试数据的分布统计未被充分利用的问题，本项目计划研究一种轻量高效的缓存模型，用于记录在线测试数据的特征和分布特性用作未来参考，其中重点和难点在于如何根据在线测试数据流构建缓存模型并设计动态更新机制。
\end{itemize}


\begin{itemize}
    \item 研究内容三：开放环境下缓存模型驱动的3D点云自适应分析方法。建立起缓存模型后，缓存模型中维护的点云样本特征和标签信息可作为知识锚点，本项目计划研究一种检索增强技术，根据当前输入样本从缓存模型中检索相关领域知识，利用检索到的知识更新和校正3D点云分析模型原本预测，实现测试时自适应目标。
\end{itemize}
...
针对开放环境下检索驱动的3D点云测试时自适应分析方法研究任务，本项目拟解决下列三个关键科学问题：

\begin{itemize}
    \item 开放环境下点云特征的高效提取与鲁棒表示。
\end{itemize}
一方面，现有方法仅提取点云数据的全局结构特征，未考虑更细致的局部细节，这并不能很好刻画点云数据特点；另一方面，直接加入全部局部数据使得点云处理方法的运行效率大幅降低而存储成本急剧增加。因此，研究一种既能高效进行点云特征编码又能全面刻画数据特点的方法，是一个关键科学问题。

\begin{itemize}
    \item 开放环境下基于在线点云数据流的缓存模型动态构建和更新。
\end{itemize}
现有方法针对测试数据流中的每个样本单独生成预测，并未考虑数据流的统计信息和分布特性，也未利用这些信息辅助模型做出针对性调整，因此整体准确性不够。
虽然有些方法也提出缓存一些典型样本用作模型测试时参考，但是这些缓存模型的建立依赖大量训练样本，且建立之后缺乏动态更新策略，无法适应开放环境下在线测试数据的变化和分布不确定性。根据在线点云数据流建立缓存模型并设计配套更新机制，有望捕获测试数据本身的统计信息和分布特性，从而辅助模型更好适应真实环境。
因此，如何在开放环境中根据在线点云数据流构建缓存模型，充分挖掘测试数据的分布特性，并为缓存模型设计动态更新机制是一个关键科学问题。

\begin{itemize}
    \item 开放环境下基于缓存模型的点云处理测试时自适应机制。
\end{itemize}
% 测试时自适应机制，这里的科学问题是啥
%   基于缓存模型的测试时自适应
现有测试时自适应方法依赖参数更新进行知识学习，虽然有一定效果，但这种模式运行效率低下无法满足部署和推理要求。另一方面，现有方法对模型在测试时的校正机制研究尚不充分，多数方法简单拼接测试时自适应模块的输出，信息融合效果不理想。
本项目提出的缓存模型是一个在线测点云数据的动态知识库，知识库的构建无需参数更新和训练，不仅极为高效，而且包含了丰富的测试数据分布特性。因此，如何基于该动态缓存模型加强开放环境下点云处理在测试阶段的自适应能力，是一个关键科学问题。
...
\subsubsection{分层缓存模型建立和更新}

为了更全面刻画测试数据分布，本项目拟开发一种分层缓存模型来记录在线测试样本的特性。
分层缓存模型包含一个全局缓存和一个局部缓存，前者存放点云样本整体信息，后者存放局部结构信息。这种由粗到细的结构能更好为测试数据进行特征画像，从而提供更精确鲁棒的特征表示。拟设计的分层模型可分如下步骤构建，参见算法~\ref{alg:hierar_cache_construction}。
...
构建：全局缓存按照样本类别存放物体，每个类别最多存放 $K$ 个物体。开始时，全局缓存为空，随着在线数据流的输入，为当前样本 $P$ 生成指纹三元组 ($\textbf{e}_p^g, \hat{L}, h$)并放置到类别$\hat{L}$中。
...
尽管全局缓存包含了测试数据的整体分布信息，但缺乏对于点云局部结构的掌握，而这些局部结构信息对于物体识别、语义分割等点云分析任务比较关键。本项目拟设计局部缓存模型以解决这种限制。

局部缓存用 $\textbf{C}_l = (\textbf{E}_l, \hat{\textbf{L}}_l)$ 表示，存储点云数据的局部特征和标签信息。与此对应，当个测试样本$P$的局部指纹信息表示为 ($\textbf{e}_p^l, \hat{L}$)，$\textbf{e}_p^l \in \mathbb{R}^{m\times d}$。

构建与更新：局部缓存的样本选择机制遵循全局缓存的设计，即仅当测试样本的全局指纹被放入全局缓存时，它的局部指纹才能被放入局部缓存。因此，在局部指纹中不必重复存储对应样本的熵值$h$。当局部缓存存满时，$\textbf{E}_l \in \mathbb{R}^{(m\cdot C K)\times d}$，$\hat{L} \in \mathbb{R}^{CK \times 1}$。

\subsubsection{基于缓存模型的测试时自适应}

构建的分层缓存模型相当于一个具有丰富点云知识的在线知识库，本项目计划用该知识库来在线校准点云处理模型的预测，有效解决测试时发生的分布偏移、数据噪声问题。测试时自适应过程分为如下两个阶段：

\myEmph{（1）基于全局缓存的测试时自适应}

对于新来的测试样本 $Q$，它被当作一个查询来从全局缓存检索相关知识。具体来说，检索过程先要计算一个权重矩阵 $A_g$，这个矩阵表示 $Q$ 和缓存特征 $\textbf{E}_g$ 的相似度。这让新的测试样本与全局缓存数据建立了联系，更意味着初步理解了线测试数据的上下文。然后利用$A_g$对缓存的测试数据标签信息$\hat{L}_g$生成全局缓存的自适应信号 $\hat{\textbf{y}}_g$。上述过程可以用公式\ref{eq:global_cache_pred}表示。
...
这里 $\textbf{e}_q^g \in \mathbb{R}^{1\times d}$ 表示样本$Q$的全局特征，$\textbf{E}_{g} \in \mathbb{R}^{CK\times d}$ 表示缓存的特征（键值），`OH' 是一种把 $\hat{\textbf{L}}_{g}$ 转换成 one-hot 编码的数据变换。$\textbf{e}_q^g \textbf{E}_{g}^\top \in \mathbb{R}^{1\times CK}$ 计算了查询与键值的余弦相似度；$\beta_g$ 是一个超参数，用来调整变换的锐度；指数运算确保权重矩阵 $A_g$ 中的值落在区间 (0,1]。$A_g$ 本质上包含了查询和缓存键值的相关关系，它作用到 $\hat{\textbf{L}}_{g}$ 用来生成全局缓存的自适应信号。

\myEmph{（2）基于局部缓存的测试时自适应}

尽管全局缓存生成的自适应信号对于模型适配测试数据有帮助，但是不够全面，因为全局缓存本身不包含点云数据的局部结构信息，这些信息对于测试数细微的结构变化很关键。为了实现更精准全面的自适应，本项目也利用局部缓存生成自适应信号。

首先，对于新的测试样本 $Q$，提取它的局部结构特征 $\textbf{e}_q^{l}$，然后该特征被当作另一个查询从局部缓存检索相关知识。类似地，检索过程先计算一个权重矩阵 $A_{l}$，它表示 $\textbf{e}_q^{l}$ 和 $\hat{\textbf{E}}_{l}$ 的相似度，然后 $A_{l}$ 加权作用到局部缓存的标签数据 $\hat{\textbf{L}}_{l}$产生局部缓存的自适应信号 $\hat{\textbf{y}}_{l}$，如公式\ref{eq:local_cache_pred}所示。
...
$\textbf{e}_q^{l} \in \mathbb{R}^{m\times d}$, $\textbf{E}_{l} \in \mathbb{R}^{(m\cdot CK)\times d}$。函数 $\phi(\cdot)$ 是一个对样本$Q$的$m$个零部件特征的平均池化操作。$e_q^{l} \textbf{E}_{l}^\top \in \mathbb{R}^{m\times (m\cdot CK)}$ 计算了样本$Q$的局部特征和缓存局部特征的余弦相似度。类似地，$\beta_l$ 也是控制锐度的超参数。$A_{l} \hat{\textbf{L}}_{l} \in \mathbb{R}^{m\times C}$ 表示样本$Q$各局部结构的类别分布，由 $\phi(\cdot)$ 取均值后得到局部缓存的自适应信号。

\myEmph{（2）在线点云数据整体测试时自适应}

经过上述步骤建立的动态分层缓存模型，能够有效且全面捕获在线测试数据特点，并实时对模型初始预测 $\hat{\textbf{y}}_{zs}$ 进行自适应调整，产生最终的模型预测 $\hat{\textbf{y}}_g$，如公式\ref{eq:overall_pred}所示，其中$\alpha_{g}$ 和 $\alpha_{l}$是超参数。
...
本项目设定的题目和研究方案建立在申请人的前期工作和对领域文献广泛深入调研的基础上，是对前期工作的拓展和深化，具体分析如下：
...
申请人第一作者在计算机视觉和人工智能领域顶级期刊和会议发表了系列研究成果，如表\ref{tab:research_foundation}所示，
在本项目涉及的开放环境3D点云分析方法、高效的点云编码模型架构设计、点云处理模型泛化性和鲁棒性提升、测试时自适应技术等方面进行了扎实的科研积累。
本人作为项目成员参与了国家自然科学基金项目《基于模块拼接的群智SLAM关键问题研究》和公安部重点实验室项目《高精点云地图构建与视觉定位导航方法研究》，为研究方案的设计、验证、执行贡献了力量。
...
\begin{table}[ht]
   ...
   \begin{tabular}{l c c c}
      ...
      \midrule
      \multirow{4}{*}{CVPR 2025} & \multirow{4}{*}{第一作者} & Point-Cache: Test-time Dynamic & 1. 3D点云分析 \\
      & & and Hierarchical Cache for  & 2. 开放环境/3D大模型 \\
      & & Robust and Generalizable & 3. 缓存模型/检索增强/即插即用 \\
      & & Point Cloud Analysis & 4. 鲁棒性/泛化性 \\
      \midrule
      \multirow{4}{*}{NeurIPS 2024} & \multirow{4}{*}{第一作者} & Point-PRC: A Prompt Learning   & 1. 3D点云分析 \\
      & & Based Regulation Framework   & 2. 开放环境/3D大模型 \\      
      & & for Generalizable & 3. 约束框架/即插即用 \\
      & & Point Cloud Analysis & 4. 准确性/泛化性 \\
      \midrule
      \multirow{4}{*}{TVCG 2025} & \multirow{4}{*}{第一作者} & VSFormer: Mining Correlations & 1. 3D形状分析 \\
      & & in Flexible View Set & 2. 封闭环境 \\
      & & for Multi-View & 3. 灵活高效的模型架构 \\
      & & 3D Shape Understanding & 4. 准确性/运行效率 \\
      \midrule
      \multirow{4}{*}{ICRA 2024} & \multirow{4}{*}{第一作者} & Parameter-efficient    & 1. 3D点云分析 \\
      & & Prompt Learning  & 2. 开放环境/3D大模型 \\
      & & for 3D & 3. 轻量提示学习 \\
      & & Point Cloud Understanding & 4. 参数效率/数据效率/准确性 \\
      \midrule
      \multirow{4}{*}{ICRA 2023} & \multirow{4}{*}{第一作者} & ViPFormer: Efficient  Transformer & 1. 3D点云理解 \\
...
综上所述，申请人具备了承担本项目的研究基础，项目设计的研究方案和技术路线具有可行性，相信在国家自然科学基金委的资助下，能够攻克项目中的重点难点问题，取得预期成果。

\NsfcSection{4}{本项目的特色与创新之处；}{}

（1）提出了融合文本和图像信息的点云特征鲁棒编码方法，为点云特征注入了语义信息，有效解决了单模态点云数据概念封闭的限制，奠定了点云处理方法在开放环境下执行任务的基础。

（2）提出了即插即用的动态分层缓存方法，该方法能够实时捕获在线测试数据分布特性，精细刻画点云数据的局部结构和全局特征，为测试时自适应分析方法的设计提供有力支撑；同时该方法是一个通用的组件，能够无缝集成到当前主流点云处理模型中，助力不同模型实现测试时准确性和泛化性提升。

（3）提出了多源信息融合的模型测试时输出校正机制，该机制充分考虑了模型的原始预测，结合了动态缓存方法根据在线测试数据特性作出的针对性调整，能够有效应对测试数据的变化和噪声，大幅改善点云处理方法在开放环境的准确性、泛化性、适应能力。

\NsfcSection{5}{年度研究计划及预期研究结果}{
（包括拟组织的重要学术交流活动、国际合作与交流计划等）。}

\subsection{年度研究计划}
2027年1月-2027年12月：（1）研究开放环境下点云特征鲁棒表示问题，设计融合文本-图像信息的点云编码架构，为点云特征注入现实世界语义信息；（2）设计实验验证点云编码架构在下游识别/检测/分割任务的高效性和准确性，验证点云特征表示在开放环境中的适配性，根据实验结果迭代优化架构设计；（3）将研究内容和技术创新撰写成论文，发表1-2篇高水平论文。

2028年1月-2028年12月：（1）研究开放环境下基于鲁棒点云特征表示的动态分层缓存构建和更新机制。（2）设计实验验证缓存模型的存储的样本特征和标签是否符合预期，验证缓存构建和更新的效率，验证缓存模型对测试时点云分析任务的提升效果，根据实验结果迭代优化缓存模型的构建和更新机制；（3）将研究内容和创新点总结凝练，发表2-3篇高水平论文，申请1项专利。

2029年1月-2029年12月：（1）研究开放环境下基于动态分层缓存的点云分析测试时自适应策略，提出不同的自适应策略；（2）设计实验评估不同测试时自适应策略对开放环境点云分析方法准确性和鲁棒性的改进效果，同时评估不同策略的运行效率，根据实验结果迭代优化，最后综合各项指标确定最佳自适应策略；（3）将研究内容和创新点撰写论文，发表2-3篇高水平论文，申请1项专利。

\subsection{学术交流合作计划}

（1）项目执行期内，申请人计划邀请领域内2-3名知名学者对本项目进行线下指导点评，让学者充分了解项目背景、研究方案、技术路线以及项目进展，进而提出比较具体的改进建议，提升项目各个环节的执行质量；同时根据本项目主题设置专门研讨会，邀请学者讲授前沿技术和发展动态，汲取各方智慧助力项目顺利推进。

（2）项目执行期内，申请人计划参加CVPR、ICCV、NeurIPS等领域内高水平国际会议，在会议现场作海报展示或者口头报告，与领域同行深入交流研究内容、技术思路和前沿进展，广泛吸收先进经验做法，为本项目的推进提供有价值参考。

\subsection{预期研究成果}

（1）理论成果：提出开放环境下鲁棒点云编码表示的模型架构和方法；提出开放环境下基于鲁棒点云表示的动态分层缓存模型；提出开放环境下基于动态分层缓存的3D点云自适应分析方法。开放环境下3D点云识别、检测、分割任务效果超越当前最佳方法。将上述创新成果撰写成论文，在TPAMI/IJCV/CVPR/ICCV/NeurIPS等高水平国际期刊和会议上发表论文5篇及以上。

（2）系统和专利：搭建1套开放环境下3D点云自适应分析演示系统，申请国家发明专利1-2项。
...
申请人自2017年开始从事基于深度学习的计算机视觉和三维视觉相关研究工作，具备扎实的理论基础和丰富的实践经验，取得了一些理论和应用创新成果。申请人的博士学位论文《多视图多模态三维物体理解关键技术研究》获评中国人民大学2025年度优秀博士学位论文，申请人于2023年获得国家留学基金委公派留学奖学金,后赴澳大利亚莫纳什大学（Monash University）数据科学与人工智能系进行为期一年的博士联合培养，联培期间的研究方向仍是三维视觉和点云处理，合作导师为蔡剑飞教授（Prof. Jianfei Cai），与蔡剑飞教授团队建立了良好而深入的合作关系。下文将从申请人的前期预研工作、参与的相关项目、获得的相关奖励三方面介绍研究基础：

\subsection{前期预研工作}

申请人博士期间围绕3D点云处理的准确性、运行效率、鲁棒性和泛化性进行深入研究，广泛阅读了相关文献，熟悉主要技术路径和方法，提出了创新性解法并发表了系列高水平论文，在该领域具备扎实的研究基础，前期工作如表~\ref{tab:research_foundation}所示。
具体来说，申请人以第一作者发表在 CVPR 2025 的论文提出《Point-Cache: Test-time Dynamic and Hierarchical Cache for Robust and Generalizable Point Cloud Analysis》提出了一个拔插即用的缓存模型，大幅改善了3D点云处理模型在测试/部署阶段的鲁棒性和泛化能力（\textbf{见附件1}）；以第一作者发表在 NeurIPS 2024 的论文《Point-PRC: A Prompt Learning Based Regulation Framework for Generalizable Point Cloud Analysis》提出了一种基于提示学习的即插即用的约束框架，该框架能无缝集成到不同多模态3D大模型中，在增强点云处理下游任务性能的同时，改善多模态3D大模型在未知点云数据的泛化能力（\textbf{见附件1}）；以第一作者发表在 TVCG 2025 的论文《VSFormer: Mining Correlations in Flexible View Set for Multi-view 3D Shape Understanding》设计了一种灵活的3D物体多视图关系建模方法，增强了不同视图间的信息流动和交互，提升了3D物体建模和识别的准确性（\textbf{见附件1}）；以第一作者发表在 ICRA 2024 的论文《Parameter-efficient Prompt Learning for 3D Point Cloud Understanding》提出了一种参数和数据双重高效的提示学习方法，仅用极少训练参数和数据样本就能激活多模态3D大模型丰富的预训练知识，显著提升了模型在点云处理任务中的准确性和运行效率（\textbf{见附件1}）；以第一作者发表在 ICRA 2023 的论文《ViPFormer: Efficient Vision-and-Pointcloud Transformer for Unsupervised Pointcloud Understanding》提出了一种无监督预训练模型架构，该架构能有效融合图像和点云多模态数据信息，促进点云处理下游任务取得更好效果，同时提升下游任务的运行效率（\textbf{见附件1}）。
...
申请人博士期间参与了和本课题相关的项目，比如《高精点云地图构建与视觉定位导航方法研究》、《基于模块拼接的群智SLAM关键问题研究》，在项目中负责文献调研和综述、点云地图构建方法设计等内容，如表\ref{tab:related_projects}所示。
通过参与这些科研项目，申请人掌握了开放环境下点云处理和自适应分析的基础理论和技术手段，具备了项目规划和执行的实际经验，初步具备了独立开展本项目的能力，相信在国家自然科学基金委的资助下，能够解决项目中的难点问题和技术挑战，按计划推进各项研究内容，实现预期研究成果。

\begin{table}[ht]
   \centering
   \caption{\textbf{申请人参与的相关项目}}
   \scriptsize
   \begin{tabular}{l c c}
    \toprule
...
申请人在空间智能和3D点云处理方面的科研工作获得了多项重要奖励，
包括：Google Travel Grant，
博士生国家奖学金，
中国人民大学优秀博士论文，
...
\begin{figure}[htbp]
     ...
     \begin{subfigure}[b]{0.32\textwidth}
         \centering
         \includegraphics[width=\textwidth]{google_travel_grant.pdf}
